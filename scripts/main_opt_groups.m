...
n_design = 3;                               % number of optimal design
...
for sub = sub_order
    % counter over the number of subjects
    counter = counter+1;
    
    path_rel = strcat('data_analysis\data_for_optimization\subject_0',...
        num2str(sub),'\session_01\trial_');
    % concatenate the data of each trial for a percentile group
    for trial = 1:n_trial
    ...
    % perform the optimisation for each percentile group
    if ~mod(counter,n_sub/n_design) 
        design = design+1;
        ...
        if display_graph
            figure;
            title_name = strcat("Design ", num2str(design), " Absolute Error");
            ...
            figure;
            title_name = strcat("Design ", num2str(design), " Relative Error");
...
%% plot percentile groups comparison histogram
if display_graph
    f = figure;
    axes1 = axes('Parent',f);
    hold(axes1,'on');
    ...
    view(30,10)
    legend('$1^{st}$ group','$2^{nd}$ group','$3^{rd}$ group','Location','NorthEast');
    title_name = "Percentile Groups' Relative Errors Comparison";
    title(title_name);
end